...
@section('content')
    ...
    @if ($feed)
        <div class="container">
            <div class="row">
                <div class="wrapper">
                    <div class="headernews">{{ $feed->title }}</div><br>
                    <div class="content">{{ $feed->abstract }}</div><br><br>
                    <div class="read-more-container">
                        <a href="{{ url('single-feed', $feed->slug) }}" class="read-more">LEER</a>
                    </div>
                    <div class="image" style="background-image: url('{{ asset('images/worship/' . $feed->image) }}');">
                    </div>
                </div>
            </div>
        </div>
            ...
            @foreach ($list as $l)
                <div class="card">
                    ...
                    <div class="card-text">
                        ...
                        <p class="card-body">{{ $l->abstract }}
                        </p>
                    </div>
                    <div class="card-link"><a href="{{ url('single-feed', $l->slug) }}">Leer</a></div>
                </div>
            @endforeach
        </div>
        <h3>{{ $list->links() }}</h3>
