...
class LookDataTable extends DataTable
{
    ...
    public function query(News $model): \Illuminate\Database\Eloquent\Builder
    {
        ...
        if (\Illuminate\Support\Facades\Auth::check() && ((\Illuminate\Support\Facades\Auth::user()->role_id ?? null) === 1)) {
            return $q->where('category', 2); // Cambiado a 2 para Mirada afro
        }
        ...
        //     return $q->where('category', 2);
        // }

        return $q->whereNull('deleted_at')->where('category', 2); // Aseguramos que solo mire categoría 2
    }

    public function html(): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('look-table') // Cambiado a look-table para diferenciar
            ...
            ->parameters([
                ...
                'initComplete' => 'function() { $("#look-table_length select, #look-table_filter input").addClass("form-control"); }',
            ]);
    }
    ...
    {
        return 'MiradaAfro_' . date('YmdHis');
    }
}