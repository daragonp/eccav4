<?php $__env->startSection('title', 'Biblia'); ?>
...
function biblia() {
  return {
    ...
    async cargarInicio() {
      try {
        const res = await fetch('<?php echo e(route("biblia.api.start")); ?>');
  ...
  this.updateLineHeight();
    ...
    async cargarLibros() {
      try {
        ...
        const res = await fetch('<?php echo e(route("biblia.api.books.organized")); ?>');
        const data = await res.json();
    ...
    async cargarCapitulos() {
      ...
      const res = await fetch(`<?php echo e(url('/biblia/api')); ?>/${this.libro}`);
    ...
    async cargarCapitulo() {
      ...
      if (this.usePagination) {
        ...
        const res = await fetch(`<?php echo e(url('/biblia/api')); ?>/${this.libro}/${this.cap}`);
        const data = await res.json();
    ...
    async loadPaginatedChapter(page = 1) {
      ...
      const res = await fetch(`<?php echo e(url('/biblia/api')); ?>/${this.libro}/${this.cap}/page/${page}`);
      const data = await res.json();
    ...
    async buscar(page = 1) {
      try {
        ...
        let url = `<?php echo e(route('biblia.api.search')); ?>?q=${encodeURIComponent(q)}`;
        if (page > 1) {
          url += `&page=${page}`;
        }
...
<?php echo $__env->make('layouts.main', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/eccav4/resources/views/biblia/index.blade.php ENDPATH**/ ?>