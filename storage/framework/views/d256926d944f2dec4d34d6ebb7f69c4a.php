

<section class="links full-bleed" aria-labelledby="social-links-title">
  <h2 id="social-links-title" class="sr-only">Enlaces sociales y recursos</h2>
  
  
  <div class="links-grid">
    
    <a href="https://tunein.com/embed/player/s292735/" target="_blank" rel="noopener" class="card" aria-label="Radio ECCA en TuneIn (se abre en nueva pestaña)">
      <img src="<?php echo e(asset('images/brands/radio.png')); ?>" alt="Radio" class="icon-img">
      <p>Radio ECCA</p>
    </a>

    <a href="#" target="_blank" rel="noopener" class="card" aria-label="Apoyo (se abre en nueva pestaña)">
      <img src="<?php echo e(asset('images/brands/support.png')); ?>" alt="Apoyo" class="icon-img">
      <p>Apoyo</p>
    </a>

    <a href="https://www.youtube.com/@tezbrillante" target="_blank" rel="noopener" class="card" aria-label="Canal de YouTube @tezbrillante (se abre en nueva pestaña)">
      <img src="<?php echo e(asset('images/brands/youtube.png')); ?>" alt="YouTube" class="icon-img">
      <p>@tezbrillante</p>
    </a>

    <a href="https://www.instagram.com/tez_brillante/" target="_blank" rel="noopener" class="card" aria-label="Instagram @tez_brillante (se abre en nueva pestaña)">
      <img src="<?php echo e(asset('images/brands/instagram.png')); ?>" alt="Instagram" class="icon-img">
      <p>@tez_brillante</p>
    </a>

    <a href="<?php echo e(url('worship')); ?>" target="_blank" rel="noopener" class="card" aria-label="Audios del culto dominical (se abre en nueva pestaña)">
      <img src="<?php echo e(asset('images/brands/worship.png')); ?>" alt="Culto dominical" class="icon-img">
      <p>Culto dominical</p>
    </a>

    
    <a href="<?php echo e(url('biblia')); ?>" class="card" aria-label="Consulta la biblia en línea">
      <img src="<?php echo e(asset('images/brands/bible.png')); ?>" alt="Biblia" class="icon-img">
      <p>La Biblia</p>
    </a>

  </div>
</section><?php /**PATH /Applications/XAMPP/xamppfiles/htdocs/eccav4/resources/views/social.blade.php ENDPATH**/ ?>